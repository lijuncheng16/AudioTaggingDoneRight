...
mean=-29.072655
std=40.71728
suffix=resnet_no-pretrain-small-feat
exp_dir=./exp/${model}-${dataset}-${set}-p$imagenetpretrain-b$batch_size-lr${lr}-fm${freqm}-tm${timem}-mix${mixup}-m${mean}-std${std}-epoch${epoch}-att_act${att_activation}-${suffix}
expid=${model}-${set}-pre${imagenetpretrain}-b${batch_size}-lr${lr}-mix${mixup}-freqm${freqm}-timem${timem}-m${mean}-std${std}-att_act${att_activation}
...
--eff_b $eff_b --att_head ${att_head} --att_activation ${att_activation} --imagenet_pretrain $imagenetpretrain" >> $logger
CUDA_CACHE_DISABLE=1 python -W ignore ../../src/run.py --model ${model} --dataset ${dataset} --n_mels ${n_mels} \
--data-train ${tr_data} --data-val ${te_data} --exp-dir $exp_dir --mean ${mean} --std ${std} \
--label-csv ./data/class_labels_indices.csv --n_class 527 \
--lr $lr --n-epochs ${epoch} --batch-size $batch_size --save_model True \
--freqm $freqm --timem $timem --mixup ${mixup} --bal ${bal} \
--eff_b $eff_b --att_head ${att_head} --att_activation ${att_activation} --imagenet_pretrain $imagenetpretrain
